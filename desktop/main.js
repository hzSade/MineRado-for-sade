...
/**
 * Параллельно выполняет stat для файлов локальной библиотеки. При большой библиотеке последовательный await растянул бы импорт папки надолго, поэтому здесь ограничен параллелизм, чтобы не забить очередь диска.
 * @param {string} root Разрешённая корневая папка локальной музыкальной библиотеки.
 * @param {Array<{abs:string, rel:string, entry:{name:string}, ext:string, index:number, source?:object}>} items Файлы, для которых нужно прочитать метаданные.
 * @returns {Promise<Array<object>>} Описание файлов, которое можно сразу вернуть во фронтенд.
 */
async function statLocalLibraryFiles(root, items) {
  const files = [];
  let cursor = 0;
  /**
   * Читает метаданные файлов, используя общий курсор; курсор синхронно увеличивается в рамках текущего цикла событий и не влияет на итоговую сортировку.
   * @returns {Promise<void>} Разрешается, когда текущий воркер завершил работу.
   */
  ...
  for (const item of Array.isArray(bindings) ? bindings : []) {
    ...
    if (registered) {
      ...
      results.push({
        ...
        ok: false,
        conflict: {
          sourceName: 'Система / другая программа',
          sourceIcon: 'warning',
          reason: 'Эта комбинация клавиш уже занята или зарезервирована системой',
        },
      });
    }
  }
...
/**
 * Читает файл настроек оболочки рабочего стола. Политика закрытия в трей должна применяться раньше загрузки фронтенда, поэтому она хранится в основном процессе.
 * @returns {{closeToTray?: boolean}} Сохранённые настройки оболочки рабочего стола.
 */
...
/**
 * Записывает файл настроек оболочки рабочего стола. В нём хранится только то поведение окна, которое основной процесс должен знать заранее.
 * @param {{closeToTray?: boolean}} patch Поля настроек, которые нужно перезаписать.
 * @returns {{closeToTray?: boolean}} Полные настройки после записи.
 */
...
/**
 * Применяет сохранённые настройки оболочки рабочего стола, чтобы поведение кнопки закрытия было определено ещё до создания окна.
 * @returns {void}
 */
...
/**
 * Читает статус автозапуска в Windows; и в режиме разработки, и в собранном приложении используется API автозагрузки Electron.
 * @returns {boolean} Запускается ли Mineradio автоматически при входе в текущую учётную запись.
 */
...
/**
 * Включает/выключает автозапуск в Windows. При ошибке выбрасывает исключение, чтобы IPC вернул понятную ошибку.
 * @param {boolean} enabled Включить ли автозапуск при старте системы.
 * @returns {{ok:boolean, enabled:boolean}} Фактическое состояние после применения настройки.
 */
...
/**
 * Пересобирает меню трея на основе текущего состояния, чтобы галочки в меню соответствовали реальным настройкам.
 * @returns {void}
 */
function refreshTrayMenu() {
  if (!tray) return;
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Показать Mineradio', click: focusMainWindow },
    {
...
/**
 * Создаёт значок в системном трее. Трей используется для восстановления окна, переключения сворачивания в трей и автозапуска.
 * @returns {void}
 */
function createTray() {
...
/**
 * Открывает официальную страницу Mineradio на GitHub либо страницу входа
 * Яндекс OAuth, не позволяя фронтенду вызывать произвольные локальные
 * протоколы или внешние сайты.
 * @param {string} rawUrl Внешняя ссылка, которую запросил открыть фронтенд.
 * @returns {boolean} true, если ссылка была открыта.
 */
function openAllowedExternalUrl(rawUrl) {
  try {
    ...
    const isGithubProjectPage = (host === 'github.com' || host === 'www.github.com') && /^\/oirge\/Mineradio(?:\/|$)/i.test(parsed.pathname);
    // Яндекс может вернуть verification_url на разных доменах (.ru/.com/ya.ru)
    // в зависимости от региона — разрешаем весь семейство официальных доменов OAuth.
    const isYandexOauthPage = host === 'oauth.yandex.ru' || host === 'oauth.yandex.com' || host === 'ya.ru' || host.endsWith('.yandex.ru') || host.endsWith('.yandex.com');
    if (!isGithubProjectPage && !isYandexOauthPage) return false;
...
ipcMain.handle('mineradio-export-json-file', async (event, payload = {}) => {
  try {
    ...
    const result = await dialog.showSaveDialog(owner, {
      title: 'Экспорт архива Mineradio',
...
ipcMain.handle('mineradio-local-music-choose-folder', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: 'Выбор папки с локальной музыкой',
...
ipcMain.handle('mineradio-open-external-url', (_event, url) => {
  const opened = openAllowedExternalUrl(url);
  return { ok: opened };
});
